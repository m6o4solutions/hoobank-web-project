...
const FeatureCard = ({ icon, title, content, index }) => (
	<div className={`flex flex-row rounded-[20px] p-6 ${index !== features.length - 1 ? 'mb-6' : 'mb-0'} feature-card`}>
		...
		<div className="ml-3 flex flex-1 flex-col">
			<h4 className="mb-1 font-poppins text-[18px] font-semibold leading-[23.4px] text-white">{title}</h4>
			<p className="font-poppins text-[16px] font-normal leading-[24px] text-dimWhite">{content}</p>
		</div>
	</div>
);
